...
df_media = pd.read_excel(main_file_path, sheet_name="media", header=None, index_col=None)
df_toc = pd.read_excel(main_file_path, sheet_name="toc", header=None, index_col=None)
...
for j in range(4, r_count):
    ...
    if id in id_image_map:
        ...
        for i in range(len(images)):
            ...
            if "00849_岩波書店『知的生産の技術』" in img_url:

                # temporal
                info_url = img_url.replace(".jpg", "/info.json").replace("/image/original/", "/images2/")
